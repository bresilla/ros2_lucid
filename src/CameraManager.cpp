...
#include <ros2_lucid/CameraManager.hpp>
...
CameraManager::CameraManager() : Node("camera_manager") {
    camera_info_sub = this->create_subscription<ros2_lucid::msg::CameraDeviceArray>("/cam_discovered", 10, std::bind(&CameraManager::cam_info_update, this, std::placeholders::_1));
    ...
    for (auto& cam : camset::by_mac) {
        ...
        auto device = ros2_lucid::msg::CameraDevice();
...
CameraManager::~CameraManager() {
    RCLCPP_INFO(this->get_logger(), "CameraManager shutting down");
}


void CameraManager::cam_info_update(const ros2_lucid::msg::CameraDeviceArray::SharedPtr msg) {
    cam_discovered = msg;
    has_cam_discovered = true;
}

void CameraManager::cam_check_update() {
    if (!has_cam_discovered) {
    ...
    for (auto camera_node : camera_devices) {
        ...
        auto request = std::make_shared<ros2_lucid::srv::AskCamera::Request>();
        request->camera_name = camera_node.name;
        auto result = camera_get->async_send_request(request, [this, camera_node](rclcpp::Client<ros2_lucid::srv::AskCamera>::SharedFuture future) {
            try {
...
void CameraManager::state_controller(const ros2_lucid::msg::CameraDevice camera, lifecycle_msgs::msg::State curr_state, bool running) {
...
unsigned int CameraManager::get_state(ros2_lucid::msg::CameraDevice camera_node, std::chrono::seconds timeout = 3s) {
...
bool CameraManager::change_state(ros2_lucid::msg::CameraDevice camera_node, std::uint8_t transition, std::chrono::seconds timeout = 3s){
